...
- (void)initLevelArray {
    ...
    int winNumTmp = 5000;
    for (int i = 0; i < 5; i++) {
        baseNumTmp = baseNumTmp + 5;
        winNumTmp = winNumTmp + 1000;
    [_levelArray addObject:[LLDGameLevel modelWithBase:LLDPlayBaseModelStep baseNum:baseNumTmp win:LLDPlayWinModelScore winnum:winNumTmp tip:@"通过连接相同颜色在规定步数内获得指定分数！"]];
    }
}
